Shader "Custom/GerstnerWater"
{
    Properties
    {
        ...
        _FoamThreshold("Foam Threshold", Range(0, 1)) = 0.9
        _FoamSoftness("Foam Softness", Range(0.01, 0.5)) = 0.1
        _FoamIntensity("Foam Intensity", Range(0, 2)) = 1.0
        _FoamNoiseScale("Foam Noise Scale", Float) = 15
        _FoamNoiseStrength("Foam Noise Strength", Range(0, 0.5)) = 0.1
    ...
    SubShader
    {
        ...
        Pass
        {
                ...
                inout float totalSteepness)
            {
                ...
                // 泡用：波の急峻さを累積（波の頂点で高くなる）
                totalSteepness += steepness * (1.0 + sinPhase) * 0.5;
            }
                ...
                out float steepness)
            {
                ...
                CalculateGerstnerWave(position, time, _Dir1.xy, _Wavelength1, _Amplitude1, _Steepness1, _Speed1, displacement, normal, steepness);
                CalculateGerstnerWave(position, time, _Dir2.xy, _Wavelength2, _Amplitude2, _Steepness2, _Speed2, displacement, normal, steepness);
                CalculateGerstnerWave(position, time, _Dir3.xy, _Wavelength3, _Amplitude3, _Steepness3, _Speed3, displacement, normal, steepness);
                CalculateGerstnerWave(position, time, _Dir4.xy, _Wavelength4, _Amplitude4, _Steepness4, _Speed4, displacement, normal, steepness);
            ...
            // 泡マスク計算
            // ============================================
            float CalculateFoamMask(float waveHeight, float steepness, float2 noiseUV, float time, float totalAmplitude)
            {
                // 高さベースの泡
                float normalizedHeight = (waveHeight / totalAmplitude) * 0.5 + 0.5;
                
                // 急峻さベースの泡
                float steepnessFactor = steepness;
                
                // 両方を組み合わせる
                float foamFactor = max(normalizedHeight, steepnessFactor);
                
                // ノイズで変化をつける
                float noise = GradientNoise(noiseUV + time * 0.1, _FoamNoiseScale);
                foamFactor += (noise - 0.5) * _FoamNoiseStrength * 2.0;
                
                // しきい値でマスク生成
                float foamMask = smoothstep(_FoamThreshold, _FoamThreshold + _FoamSoftness, foamFactor);
                
                return saturate(foamMask * _FoamIntensity);
            }
            ...
            Varyings vert(Attributes input)
            {
                ...
                float3 waveNormal;
                float waveSteepness;
                CalculateGerstnerWaves4(positionWS.xz, _Time.y, displacement, waveNormal, waveSteepness);
                ...
                output.steepness = waveSteepness;
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                // 水の色と反射を混合
                float3 baseColor = lerp(waterColor.rgb, reflectionColor, fresnel);
                
                // ============================================
                // 泡の計算
                // ============================================
                float foamMask = CalculateFoamMask(
                    input.waveHeight, 
                    input.steepness, 
                    input.noiseUV, 
                    _Time.y, 
                ...
                // 泡の色を混合
                float3 finalColor = lerp(baseColor, _FoamColor.rgb, foamMask);
                ...
                // 泡にも少しスペキュラを残す（濡れた感じ）
                float foamSpecular = pow(NdotH, 32.0) * 0.3 * foamMask;
                
                // 最終色
                float3 color = diffuse + (specular + sparkle + foamSpecular) * mainLight.color;
                
                // フォグ適用
                color = MixFog(color, input.fogFactor);
                
                // アルファ（泡の部分は不透明に）
                float alpha = lerp(_Alpha, 1.0, max(fresnel * 0.5, foamMask));
        ...
        Pass
        {
                ...
                inout float3 displacement)
            {
                ...
                displacement.x += Q * amplitude * dir.x * cos(phase);
                displacement.y += amplitude * sin(phase);
                displacement.z += Q * amplitude * dir.y * cos(phase);
            }
